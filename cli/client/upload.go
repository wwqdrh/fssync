...
func newUploadView(stateFn func(state int) *clientView) uploadView {
	c := uploadView{
		inputs:  make([]field, 3),
		stateFn: stateFn,
	}
	...
	for i := range c.inputs {
		...
		case 1:
			t.field.Placeholder = " 上传的文件: ./example.txt"
			t.id = "upload"
		}
		c.inputs[i] = t
	}
...
func (c uploadView) Init() tea.Cmd {
	return textinput.Blink
}

func (c uploadView) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		case "ctrl+c", "esc":
...
func (c *uploadView) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(c.inputs))
